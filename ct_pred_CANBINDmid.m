...
SHAPS=readtable('data\tabular\Clinical_SHAPS_MDD_BSLN\CBN01_SHAPS_DATA_Z3_01_V01_TRTMT.csv');
...
demo_CANBIND=innerjoin(demo, MADRS_BL);demo_CANBIND=innerjoin(demo_CANBIND, MADRS_FU);demo_CANBIND=innerjoin(demo_CANBIND, SHAPS, 'Keys','SUBJLABEL');  demo_CANBIND=innerjoin(demo_CANBIND, BMI, 'Keys','SUBJLABEL');demo_CANBIND=innerjoin(demo_CANBIND, GAD7, 'Keys','SUBJLABEL');
...
load("globalFC_canbind.mat"); %load('globalsubFC_embarc.mat'); l=reshape(globalsubFC(:,1,:), [157 180]);r=reshape(globalsubFC(:,6,:), [157 180]); globalfFC=[l,r];
      roi=[ 179 180 359 360  ]; for i=1:157; globalfFC(i,:)=nanmean(Fnetmats_all(i,roi,:)); end % 221   252   270 241 %   68    90    98   108   165   221   240   241   245   252   270   345
       %108 114 %0 35 64 66 76 91 93   115   146   161   165   180   197   216   241   253   272   283   292   307 322   335   360
       %not bad: 64 66 76      % OR 61 180 241 360      % OR 61 179 180 241 359 360        %  62 179 180 242 359 360     % 61 62 179 180 241  242 359 360            35    64    66    91    93   108   115   161   180  197   216   272   283   307   335   360
tmp=subjects;       
...
cd C:\Users\peter\Documents\CANBIND\reports\
CTl=readtable('C:\Users\peter\Documents\CANBIND\reports\lh.aparc_stats.txt');CTr=readtable('C:\Users\peter\Documents\CANBIND\reports\rh.aparc_stats.txt');CTr.rh_aparc_thickness=[];CTl.lh_aparc_thickness=[];
subjects=horzcat(CTl, CTr);subjects.lh_MeanThickness_thickness=[];subjects.rh_MeanThickness_thickness=[];
aseg=readtable('C:\Users\peter\Documents\CANBIND\reports\aseg_stats.txt');
...
subjects.ID=aseg.Measure_volume;
subjects.qc=aseg.SurfaceHoles>380;

subjects.SUBJLABEL=subjects.ID;
subjects.SUBJLABEL=replace(subjects.SUBJLABEL,'sub-','');
...
load('globalFC_embarc.mat');  % load('globalsubFC_embarc.mat');l=reshape(globalsubFC(:,1,:), [330 180]);r=reshape(globalsubFC(:,6,:), [330 180]); globalfFC=[l,r];
...
tmp=subjects;
subjects2=table();globalfFC(globalfFC(:,1)==0,:)=NaN;
subjects2.rsfmri=globalfFC;
subjects2.ProjectSpecificID=tmp;
subcort_rois={'L_accumbens', 'L_amygdala', 'L_caudate', 'L_hippocampus', 'L_putamen','R_accumbens', 'R_amygdala', 'R_caudate', 'R_hippocampus', 'R_putamen'};

clindata=readtable('C:\Users\peter\Documents\EMBARC\data\clinical_data_fromChristianW.xlsx');clindata(clindata.delete==1, :)=[];
%clindata=innerjoin(clindata, subjects2);

CTl=readtable('C:\Users\peter\Documents\EMBARC\reports\lh.aparc_stats.txt');CTr=readtable('C:\Users\peter\Documents\EMBARC\reports\rh.aparc_stats.txt');CTr.rh_aparc_thickness=[];CTl.lh_aparc_thickness=[];
subjects=horzcat(CTl, CTr);subjects.lh_MeanThickness_thickness=[];subjects.rh_MeanThickness_thickness=[];
aseg=readtable('C:\Users\peter\Documents\EMBARC\reports\aseg_stats.txt');
subjects.lh_hippo=aseg.Left_Hippocampus./aseg.EstimatedTotalIntraCranialVol*1000;subjects.rh_hippo=aseg.Right_Hippocampus./aseg.EstimatedTotalIntraCranialVol*1000;
subjects.lh_amygdala=aseg.Left_Amygdala./aseg.EstimatedTotalIntraCranialVol*1000;subjects.rh_amygdala=aseg.Right_Amygdala./aseg.EstimatedTotalIntraCranialVol*1000;
subjects.lh_striatum=aseg.Left_Accumbens_area./aseg.EstimatedTotalIntraCranialVol+aseg.Left_Caudate./aseg.EstimatedTotalIntraCranialVol+aseg.Left_Putamen./aseg.EstimatedTotalIntraCranialVol;subjects.lh_striatum=subjects.lh_striatum*1000;
subjects.rh_striatum=aseg.Right_Accumbens_area./aseg.EstimatedTotalIntraCranialVol+aseg.Right_Caudate./aseg.EstimatedTotalIntraCranialVol+aseg.Right_Putamen./aseg.EstimatedTotalIntraCranialVol;subjects.rh_striatum=subjects.rh_striatum*1000;
...
dat = demo_CANBIND{:, 100:173}';
age = demo_CANBIND.AGE; sex = demo_CANBIND.SEX;sex = dummyvar(sex);
mod = [age sex(:,2)];
full_harmonized = combat(dat, batch, mod, 1); full_harmonized=full_harmonized';
demo_CANBIND{:, 100:173}=full_harmonized;
...
%% Model Train in CANBIND
%anova1(madrs_cut.RC_Z, madrs_cut.remission);anova1(madrs_cut.baseline_madrs_scr, madrs_cut.remission);
Xlogist=[demo_CANBIND{:, 100:173}, demo_CANBIND.SHAPS_Tot, demo_CANBIND.AGE, demo_CANBIND.SEX, madrs2hdrs17(demo_CANBIND.MADRS_TOT_PRO_RATED)',...
    (madrs2hdrs17(demo_CANBIND.MADRS_2wks)-madrs2hdrs17(demo_CANBIND.MADRS_TOT_PRO_RATED))'./madrs2hdrs17(demo_CANBIND.MADRS_TOT_PRO_RATED)', demo_CANBIND.EMPLOY_STATUS==1 , demo_CANBIND.BL_BMI];
varnames=[demo_CANBIND.Properties.VariableNames(100:173),{'shaps'}, {'age'}, {'sex'}, {'blmadrs'},{'MADRSchange'},{'employ'}, {'bmi'}];%, 'tmt5','rsm','rsr'
Ylogist=demo_CANBIND.response==1; 
%Xlogist(isnan(Ylogist),:)=[]; Ylogist(isnan(Ylogist))=[]; %% 149:150
...
save('test_coef_ct.mat','coef')
%test_can_in_embarc

%% MODEL TEST IN EMBARC
%%%%%%%%%%%step 1 sert
Xlogist=[clin_ct{:,1:74}, clin_ct.shaps, clin_ct.age, clin_ct.sex, clin_ct.w0_score_17, (clin_ct.w2_score_17-clin_ct.w0_score_17)./clin_ct.w0_score_17,   clin_ct.employ<=2, clin_ct.bmi   ];
%varnames=[clin_ct.Properties.VariableNames(1:74),{'shaps'}, {'age'}, {'sex'},{'blmadrs'},{'employ'}];%, 'tmt5','rsm','rsr'
...
%       Ylogist(isnan(Xlogist(:,1)) )=[]; Xlogist(isnan(Xlogist(:,1) ),:)=[]; Xlogist(:,roi)=[];varnames(roi)=[];
yhat = glmval(coef,Xlogist,'logit');
[X,Y,T,AUCstep1EMB] = perfcurve(Ylogist,yhat, 1);AUCstep1EMB %AUC_test(randfold)=AUC;
...
%%%%%%%%%%%step 2 sert
XTest=[clin_stage2{:,1:74}, clin_stage2.shaps, clin_stage2.age, clin_stage2.sex, clin_stage2.w0_score_17,  (clin_stage2.w10_score_17-clin_stage2.w0_score_17)./clin_stage2.w0_score_17,  clin_stage2.employ<=2, clin_stage2.bmi   ]; %clin_stage2.lh_inferiortemporal_thickness, clin_stage2.rh_hippo, clin_stage2.rh_parstriangularis_thickness, 
...
%%%% embarc placebo
XTest=[clin_pla{:,1:74}, clin_pla.shaps, clin_pla.age, clin_pla.sex, clin_pla.w0_score_17,   (clin_pla.w2_score_17-clin_pla.w0_score_17)./clin_pla.w0_score_17,  clin_pla.employ<=2 , clin_pla.bmi  ];%  ,clin_pla.lh_hippo,clin_pla.rh_hippo  ];%    ,clin_stage2.lh_inferiortemporal_thickness,clin_stage2.rh_inferiortemporal_thickness ];  
...
%% %% %% %%%% %%%% %%%% %%%% %%%% %%%% %% %% %% %%%% %%%% %%%% %%%% %%%% %%%% %%
load('C:\Users\peter\Documents\CANBIND\reports\test_coef_ct.mat','coef'); 
coef(1)=[];
ix=coef~=0;
Xlogist=[clin_ct{:,1:74}, clin_ct.shaps, clin_ct.age, clin_ct.sex, clin_ct.w0_score_17, (clin_ct.w2_score_17-clin_ct.w0_score_17)./clin_ct.w0_score_17,  clin_ct.employ<=2, clin_ct.bmi];%       ,clin_ct.lh_hippo,clin_ct.rh_hippo]; %  
Ylogist=strcmp(clin_ct.w8_responder,'Yes'); 
varnames=[clin_ct.Properties.VariableNames(1:74),{'shaps'},{'age'}, {'sex'}, {'blmadrs'},{'MADRSchange'}, {'employ'}, {'bmi'}];%   {'lHippo'},{'rHippo'}  ];%    ,{'linfTemp'},{'rinfTemp'}]; 
Xlogist(strcmp(clin_ct.w8_responder,''),:)=[];Ylogist(strcmp(clin_ct.w8_responder,''))=[];
Xlogist(isnan(Ylogist),:)=[]; Ylogist(isnan(Ylogist))=[]; 
varnames(ix==0)=[];Xlogist(:, ix==0)=[];
AUC=0.5;
while AUC==0.5
[B,FitInfo] = lassoglm(Xlogist,Ylogist,'binomial','CV',10, 'PredictorNames', varnames      ,'alpha', 0.0001); %     
%lassoPlot(B,FitInfo,'PlotType','CV'); legend('show','Location','best') % show legend
idxLambdaMinDeviance = FitInfo.IndexMinDeviance;
MinModelPredictors = FitInfo.PredictorNames(B(:,idxLambdaMinDeviance)~=0)
idxLambda1SE = FitInfo.Index1SE;
...
%% CANBIND test
XTest=[demo_CANBIND{:,100:173},  demo_CANBIND.SHAPS_Tot, demo_CANBIND.AGE, demo_CANBIND.SEX,...
    madrs2hdrs17(demo_CANBIND.MADRS_TOT_PRO_RATED)',  (madrs2hdrs17(demo_CANBIND.MADRS_2wks)-madrs2hdrs17(demo_CANBIND.MADRS_TOT_PRO_RATED))'./madrs2hdrs17(demo_CANBIND.MADRS_TOT_PRO_RATED)',  demo_CANBIND.EMPLOY_STATUS==1, demo_CANBIND.BL_BMI ];%   , demo_CANBIND.lh_hippo, demo_CANBIND.rh_hippo];%    
...
%% EMBARC stage 2 test
XTest=[clin_stage2{:, 1:74}, clin_stage2.shaps, clin_stage2.age, clin_stage2.sex, clin_stage2.w0_score_17,...
    (clin_stage2.w10_score_17-clin_stage2.w0_score_17)./clin_stage2.w0_score_17,  clin_stage2.employ<=2, clin_stage2.bmi  ];%    ,clin_stage2.lh_hippo,clin_stage2.rh_hippo];  %
...
%% EMBARC placebo test
XTest=[clin_pla{:, 1:74}, clin_pla.shaps, clin_pla.age, clin_pla.sex, clin_pla.w0_score_17, (clin_pla.w2_score_17-clin_pla.w0_score_17)./clin_pla.w0_score_17,   clin_pla.employ<=2, clin_pla.bmi];%    ,clin_stage2.lh_inferiortemporal_thickness,clin_stage2.rh_inferiortemporal_thickness ];  
YTest=strcmp(clin_pla.w8_responder,'Yes');
%Ylogist=clin_ct.remission_step1;
XTest(strcmp(clin_pla.w8_responder,''),:)=[];YTest(strcmp(clin_pla.w8_responder,''))=[];
XTest(:, ix==0)=[];
