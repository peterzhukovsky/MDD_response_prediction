...
%% BRAIN DISTR TRAJECTORIES
    %% RIGHT SIDE first
    clear full_cell
    id=[1:180]'; 
    for roi=1:length(rlabel_names)
        if T(roi)==1 
        R(roi,1)=0; G(roi,1)=0; B(roi,1)=255; A(roi,1)=0;
        elseif T(roi)==2 
        R(roi,1)=255; G(roi,1)=0; B(roi,1)=0; A(roi,1)=0;
        elseif T(roi)==3 
        R(roi,1)=128; G(roi,1)=128; B(roi,1)=128; A(roi,1)=0;
        %R(roi,1)=0; G(roi,1)=204; B(roi,1)=0; A(roi,1)=0;
        %elseif T(roi)==4
        %R(roi,1)=128; G(roi,1)=128; B(roi,1)=128; A(roi,1)=0;
        end
...
%% AGE CAT 1 - all SENDEP
%%RIGHT SIDE first
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1==1 & adjr2>0 & r'>180) | (inconsistent1==1   & adjr2>0 & r'>180) | (unclear1==1 & adjr2>0 & r'>180)); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1(roi)==1
    blue_range=0:255;
    green_range=0:255;
    tstat_relative=1-abs(tstat1(roi))/(max(tstat1)+0.1);
    ...
    A(index,1)=0;
    full_cell{index, 1}=index; full_cell{index, 2}=rlabel_names{roi-180};full_cell{index, 3}=R(index);    full_cell{index, 4}=G(index);    full_cell{index, 5}=B(index);     full_cell{index, 6}=A(index);
    
    elseif consistent1(roi)==1
    red_range=0:255;
    green_range=0:255;
    tstat_relative=1-abs(tstat1(roi))/(max(tstat1)+0.1);
    color_relative=round(tstat_relative*256);
    ...
    A(index,1)=0;
    full_cell{index, 1}=index; full_cell{index, 2}=rlabel_names{roi-180};full_cell{index, 3}=R(index);    full_cell{index, 4}=G(index);    full_cell{index, 5}=B(index);     full_cell{index, 6}=A(index);
    elseif unclear1(roi)==1
    red_range=0:160; green_range=0:160;blue_range=0:160;
    tstat_relative=1-abs(tstat1(roi))/(max(tstat1)+0.1);
    color_relative=round(tstat_relative*160);
    
    R(index,1)=red_range(color_relative);
    G(index,1)=round(green_range(color_relative));
    B(index,1)=round(green_range(color_relative));
    A(index,1)=0;
    full_cell{index, 1}=index; full_cell{index, 2}=rlabel_names{roi-180};full_cell{index, 3}=R(index);    full_cell{index, 4}=G(index);    full_cell{index, 5}=B(index);     full_cell{index, 6}=A(index);
    end
end
...
%%LEFT SIDE next
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1==1 & adjr2>0 & r'<=180) | (inconsistent1==1  & adjr2>0 & r'<=180) ); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1(roi)==1
    ...
    R(index,1)=255;
    G(index,1)=round(green_range(color_relative));
    B(index,1)=round(blue_range(color_relative));
    A(index,1)=0;
    ...
    R(index,1)=round(red_range(color_relative));
    G(index,1)=round(green_range(color_relative));
    B(index,1)=255;
    A(index,1)=0;
    full_cell{index, 1}=index; full_cell{index, 2}=llabel_names{roi};full_cell{index, 3}=R(index);    full_cell{index, 4}=G(index);    full_cell{index, 5}=B(index);     full_cell{index, 6}=A(index);
...
%% PACTMD
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1rlld==1 & adjr2>0 & r'>180) | (inconsistent1rlld==1 & adjr2>0 & r'>180) | (unclear1rlld==1 & adjr2>0 & r'>180)); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1rlld(roi)==1
    blue_range=0:255;
    green_range=0:255;
    tstat_relative=1-abs(tstat1(roi))/(max(tstat1)+0.1);
    color_relative=round(tstat_relative*256);
...
%%LEFT SIDE next
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1rlld==1 & adjr2>0 & r'<=180) | (inconsistent1rlld==1 & adjr2>0 & r'<=180) ); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1rlld(roi)==1
...
%% OPT NEURO
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1trlld==1 & adjr2>0 & r'>180) | (inconsistent1trlld==1 & adjr2>0 & r'>180) | (unclear1trlld==1 & adjr2>0 & r'>180)); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1trlld(roi)==1
    blue_range=0:255;
    green_range=0:255;
    tstat_relative=1-abs(tstat1(roi))/(max(tstat1)+0.1);
    color_relative=round(tstat_relative*256);
...
%%LEFT SIDE next
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1trlld==1 & adjr2>0 & r'<=180) | (inconsistent1trlld==1 & adjr2>0 & r'<=180) ); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1trlld(roi)==1
...
%% HC all
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1hc==1 & adjr2>0 & r'>180) | (inconsistent1hc==1  & adjr2>0 & r'>180) | (unclear1hc==1 & adjr2>0 & r'>180)); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1hc(roi)==1
    blue_range=0:255;
    green_range=0:255;
    tstat_relative=1-abs(tstat1(roi))/(max(tstat1)+0.1);
    color_relative=round(tstat_relative*256);
...
%%LEFT SIDE next
clear R G B A full_cell 
r=[1:360]'; rr=r((consistent1hc==1 & adjr2>0 & r'<=180) | (inconsistent1hc==1 & adjr2>0 & r'<=180) ); 
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if inconsistent1hc(roi)==1
...
%% LASSO - beta values plotted
%%RIGHT SIDE first
clear R G B A full_cell 
r=[1:360]'; rr=r(B_ROCK(:,idxLambdaMinMSE )~=0 & r>180); beta=zscore( B_ROCK(B_ROCK(:,idxLambdaMinMSE)~=0 & r>180) );
for index=1:length(rr);
    roi=rr(index);
    %e.g. t=2; maxt=4 => then color should be half way to the max of the range:
    if beta(index)>0
    blue_range=0:186;
    green_range=0:186;
    tstat_relative=1-abs(beta(index))/(max(abs(beta))+0.1);
    color_relative=round(tstat_relative*186);
